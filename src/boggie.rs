//! boggie 模型强相关代码：资产加载（模型 + 贴图）与离屏渲染测试。
//! 结构仿照 african_head：3 个模型（body、head、eyes）各自一套
//! diffuse / 法线 / specular 贴图。
...
/// boggie 的全部资产：3 个模型 + 9 张贴图
pub struct BoggieAssets {
    /// 模型：[0]=身体, [1]=头部, [2]=眼睛（head/eyes 的坐标自带偏移，与 body 共用同一模型矩阵）
    pub models: Vec<Vec<VertexInput>>,
    /// 身体漫反射贴图 (2048²)
    pub body_diffuse_texture: TGAImage,
    /// 身体法线贴图 (2048²)
    pub body_normal_texture: TGAImage,
    /// 身体高光贴图 (2048²)
    pub body_spec_texture: TGAImage,
    /// 头部漫反射贴图 (1024²)
    pub head_diffuse_texture: TGAImage,
    /// 头部法线贴图 (1024²)
    pub head_normal_texture: TGAImage,
    /// 头部高光贴图 (1024²)
    pub head_spec_texture: TGAImage,
    /// 眼睛漫反射贴图 (256²)
    pub eyes_diffuse_texture: TGAImage,
    /// 眼睛法线贴图 (256²)
    pub eyes_normal_texture: TGAImage,
    /// 眼睛高光贴图 (256²)
    pub eyes_spec_texture: TGAImage,
}

/// 一次性加载 boggie 的全部模型与贴图（加载失败直接 panic，符合本工程惯例）
pub fn load_boggie_assets() -> BoggieAssets {
    // ---- 模型 ----
    let models: Vec<Vec<VertexInput>> = vec![
        load_model("assert/boggie/body.obj", true),
        load_model("assert/boggie/head.obj", true),
        load_model("assert/boggie/eyes.obj", true),
    ]
    .into_iter()
    .flatten()
    .collect();
    ...
    // ---- 身体贴图 ----
    let mut body_diffuse_texture = TGAImage::new(2048, 2048, RGB);
    body_diffuse_texture
        .read_tga_file("assert/boggie/body_diffuse.tga")
        .unwrap();
    body_diffuse_texture.flip_vertically();

    let mut body_normal_texture = TGAImage::new(2048, 2048, RGB);
    body_normal_texture
        .read_tga_file("assert/boggie/body_nm_tangent.tga")
        .unwrap();
    body_normal_texture.flip_vertically();

    let mut body_spec_texture = TGAImage::new(2048, 2048, RGB);
    body_spec_texture
        .read_tga_file("assert/boggie/body_spec.tga")
        .unwrap();
    body_spec_texture.flip_vertically();

    // ---- 头部贴图 ----
    let mut head_diffuse_texture = TGAImage::new(1024, 1024, RGB);
    head_diffuse_texture
        .read_tga_file("assert/boggie/head_diffuse.tga")
        .unwrap();
    head_diffuse_texture.flip_vertically();

    let mut head_normal_texture = TGAImage::new(1024, 1024, RGB);
    head_normal_texture
        .read_tga_file("assert/boggie/head_nm_tangent.tga")
        .unwrap();
    head_normal_texture.flip_vertically();

    let mut head_spec_texture = TGAImage::new(1024, 1024, RGB);
    head_spec_texture
        .read_tga_file("assert/boggie/head_spec.tga")
        .unwrap();
    head_spec_texture.flip_vertically();

    // ---- 眼睛贴图 ----
    let mut eyes_diffuse_texture = TGAImage::new(256, 256, RGB);
    eyes_diffuse_texture
        .read_tga_file("assert/boggie/eyes_diffuse.tga")
        .unwrap();
    eyes_diffuse_texture.flip_vertically();

    let mut eyes_normal_texture = TGAImage::new(256, 256, RGB);
    eyes_normal_texture
        .read_tga_file("assert/boggie/eyes_nm_tangent.tga")
        .unwrap();
    eyes_normal_texture.flip_vertically();

    let mut eyes_spec_texture = TGAImage::new(256, 256, RGB);
    eyes_spec_texture
        .read_tga_file("assert/boggie/eyes_spec.tga")
        .unwrap();
    eyes_spec_texture.flip_vertically();
...
mod tests {
    ...
    /// 离屏渲染 boggie body：画面应出现明显亮于漫反射上限的高光像素
    #[test]
    fn render_boggie_body_has_specular_highlight() {
        let verts = load_model("assert/boggie/body.obj", true).unwrap();

        let mut normal_texture = TGAImage::new(2048, 2048, RGB);
        normal_texture
            .read_tga_file("assert/boggie/body_nm_tangent.tga")
            .unwrap();
