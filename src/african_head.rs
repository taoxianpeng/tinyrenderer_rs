...
use crate::drawline::TGAImageType::{Grayscale, RGB};
...
/// african_head 的全部资产：3 个模型 + 7 张贴图
...
pub fn load_african_head_assets() -> AfricanHeadAssets {
    ...
    let mut head_spec_texture = TGAImage::new(1024, 1024, Grayscale);
    head_spec_texture
        .read_tga_file("assert/african_head/african_head_spec.tga")
        .unwrap();
...
pub fn fragment_shader(uniforms: &Uniforms, frag: &FragmentInput, _depth: &Vec<f32>) -> TGAColor {
    ...
    // 采样高光贴图：灰度值调制 specular 强度
    // （african_head_spec.tga 为 8bpp 灰度图，TGAImage::get() 把灰度值放在 b 通道，
    //   故取 r/g/b 最大值兼容灰度与 RGB 两种格式）
    let spec_mask = match uniforms.specular_tex {
        Some(spec_tex) => {
            let x = ((texcoord.x.clamp(0.0, 1.0) * spec_tex.width() as f32) as usize)
                .min(spec_tex.width() - 1);
            let y = ((texcoord.y.clamp(0.0, 1.0) * spec_tex.height() as f32) as usize)
                .min(spec_tex.height() - 1);
            match spec_tex.get(x, y) {
                Some(c) => {
...
mod tests {
    ...
    fn render_african_head_has_specular_highlight() {
        ...
        let mut spec_texture = TGAImage::new(1024, 1024, Grayscale);
        spec_texture
            .read_tga_file("assert/african_head/african_head_spec.tga")
            .unwrap();
        ...
        let uniforms = Uniforms {
            ...
            specular_tex: Some(&spec_texture),
            glossiness_tex: None,
        };
